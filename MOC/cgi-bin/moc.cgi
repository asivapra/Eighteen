...
#&debugEnv;	
	...
	else
	{
		...
		if ($sc_action eq "login")
		{
			...
			print "Content-type: text/html\n\n"; $headerAdded = 1;
			&ConnectToDBase;
			$query = "select user_id from `users` where user_email='$User_email' and password='$Password' limit 0,1";
			&execute_query($query);
...
#CREATE TABLE `addresses` (  `id` int(4) NOT NULL AUTO_INCREMENT,  `user_id` varchar(30) NOT NULL DEFAULT '',  `to_email` varchar(30) NOT NULL DEFAULT '',  `status` int(1) DEFAULT '1',  `created_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,  PRIMARY KEY (`user_id`, `to_email`),  KEY `id` (`id`)) ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=latin1; 
